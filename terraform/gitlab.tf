...
resource "kubernetes_config_map" "terraform-gitlab-info" {
  ...
  data = {
    ...
    "git-ssh-security-groups"  = aws_security_group.gitlab-ingress.id
  }
}
...
# This security group is what is used to control access to the gitlab ingress controller
resource "aws_security_group" "gitlab-ingress" {
  name        = "${var.cluster_name}-gitlab-ingress"
  description = "Allows ips into gitlab ingress for ${var.cluster_name}"
  vpc_id      = aws_vpc.eks.id
...
# This file is generated by deploy.sh and discover-gitlab-sources.sh
locals {
  gitssh-ips = yamldecode(file("/tmp/git-ssh-ips.yaml"))
}
