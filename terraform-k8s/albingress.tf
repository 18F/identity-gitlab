...
resource "aws_iam_role_policy" "alb" {
  ...
  # This came from curl -o iam_policy.json https://raw.githubusercontent.com/kubernetes-sigs/aws-load-balancer-controller/v2.1.3/docs/install/iam_policy.json
...
resource "kubernetes_config_map" "terraform-alb-info" {
  metadata {
    name      = "terraform-alb-info"
    namespace = "kube-system"
  }
